      ...
      <header className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href={'/movie/' + movie.id + '/edit'} className="flex items-center gap-1.5 bg-blue-500 text-white px-4 py-1.5 rounded-full text-xs font-medium hover:bg-blue-600 transition-colors">
              <Edit className="w-3.5 h-3.5" />
              编辑
            </Link>
            <button onClick={() => setShowConfirm(true)} disabled={deleting} className="flex items-center gap-1.5 bg-red-500 text-white px-4 py-1.5 rounded-full text-xs font-medium hover:bg-red-600 transition-colors disabled:opacity-50">
          ...
          <div className="flex flex-col md:flex-row">
            <div className="md:w-80 shrink-0 bg-gray-50">
              <div className="aspect-[2/3] relative">
                {movie.poster ? <img src={movie.poster} alt={movie.title} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300"><Film className="w-16 h-16" /></div>}
              </div>
            </div>
            <div className="flex-1 p-6 md:p-8">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{movie.title}</h1>
              {movie.year && <p className="text-sm text-gray-500 mb-4">{movie.year}</p>}
              {movie.genres && movie.genres.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-5">
                  {movie.genres.map((g) => <span key={g} className="bg-[#ff6b6b]/10 text-[#ff6b6b] text-xs px-3 py-1 rounded-full font-medium">{g}</span>)}
                </div>
              )}
              <div className="flex flex-wrap gap-6 mb-6">
                <div>
                  <div className="text-xs text-gray-400 mb-1">我的评分</div>
                ...
                <div>
                  <div className="text-xs text-gray-400 mb-1">豆瓣评分</div>
                  <div className="flex items-center gap-1.5">
              ...
              {movie.douban_url && (
                <a href={movie.douban_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm text-[#4a9eff] hover:underline mb-2">
          ...
          <div className="border-t border-gray-100 px-6 md:px-8 py-6 space-y-5">
            {movie.director && (
              <div className="flex items-start gap-3">
                ...
                <div><span className="text-xs text-gray-400">导演</span><p className="text-sm text-gray-800">{movie.director}</p></div>
              </div>
            )}
            {movie.cast && (
              <div className="flex items-start gap-3">
                <User className="w-4 h-4 text-gray-400 mt-0.5 shrink-0" />
                <div><span className="text-xs text-gray-400">主演</span><p className="text-sm text-gray-800">{movie.cast}</p></div>
              </div>
            )}
            {movie.overview && (
              <div><span className="text-xs text-gray-400">简介</span><p className="text-sm text-gray-600 mt-1 leading-relaxed">{movie.overview}</p></div>
            )}
            {movie.created_at && (
